...
namespace SafeExamBrowser.Settings.Browser
{
	...
	public class BrowserSettings
	{
		...
		/// <summary>
		/// Blinkered: the first site to open at launch (canonicalized server-side). When present, native opens this URL in a
		/// site window at the home page's document-start — concurrent with the home shell parse — instead of waiting for the
		/// home page's own window.open after 'load'. Empty/absent = do nothing (home page's normal path runs unchanged).
		/// </summary>
		public string FirstSiteUrl { get; set; }

		/// <summary>
		/// Blinkered: the per-child id used for Focus cookie isolation. When present, the Focus session restores this
		/// child's saved cookie jar on start and saves it on exit (single-child, session-lifecycle). Empty/absent =
		/// no isolation (normal shared cookie jar, do nothing).
		/// </summary>
		public string FocusChildId { get; set; }
