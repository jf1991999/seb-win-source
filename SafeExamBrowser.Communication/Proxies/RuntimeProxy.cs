...
namespace SafeExamBrowser.Communication.Proxies
{
	...
	public class RuntimeProxy : BaseProxy, IRuntimeProxy
	{
		...
		public CommunicationResult InformContentReady()
		{
			try
			{
				var response = Send(SimpleMessagePurport.ContentIsReady);
				var success = IsAcknowledged(response);

				if (success)
				{
					Logger.Debug("Runtime acknowledged that the content is ready.");
				}
				else
				{
					Logger.Error($"Runtime did not acknowledge that the content is ready! Response: {ToString(response)}.");
				}
			...
			{
				Logger.Error($"Failed to perform '{nameof(InformContentReady)}'", e);
