...
namespace SafeExamBrowser.Runtime
{
	internal class CompositionRoot
	{
		...
		// #4 fix B: the client-ready wait must tolerate a SLOW first launch after install — cold CEF + the one-time
		// Windows Defender scan of the new CefSharp.BrowserSubprocess.exe can push browser init well past 30s (worse while
		// it contends with the concurrent lockdown ops). A crashed client still fails FAST (ClientTerminated sets the gate
		// immediately); this only lengthens the wait for a live-but-slow client, so a genuine hang still fails closed.
		...
		internal void BuildObjectGraph(Action shutdown)
		{
			...
			// Launch-only full-screen black cover (best-effort — a failure here must NEVER break the launch): shown as
			// the first window so a windowed Focus launch goes straight to black (no home-desktop flash), torn down at
			// content-ready.
			IWindow launchCover = null;
			try { launchCover = uiFactory.CreateLaunchCover(appConfig); }
			catch (Exception e) { logger.Warn($"Launch cover unavailable (continuing without it): {e.Message}"); }
			...
			var responsibilities = BuildResponsibilities(integrityModule, messageBox, runtimeHost, runtimeWindow, serviceProxy, context, sessionSequence, shutdown, splashScreen, launchCover);

			context.Responsibilities = responsibilities;

			RuntimeController = new RuntimeController(logger, bootstrapSequence, responsibilities, context, runtimeWindow, splashScreen, launchCover);
		}
			...
			IWindow launchCover)
		{
			...
			responsibilities.Enqueue(new SessionResponsibility(appConfig, ModuleLogger(nameof(SessionResponsibility)), messageBox, runtimeContext, runtimeWindow, splashScreen, launchCover, runtimeHost, sessionSequence, shutdown, text));
			...
			IUserInterfaceFactory uiFactory)
		{
			...
			// === #4 launch speedup: early client spawn + explicit fail-closed AND-gate reveal ===========================
			// The client is a ~1.7s .NET cold start. Rather than starting it just before the reveal (its old position, where
			// the start blocked the whole sequence), we now: (1) ClientTermination — tear down any client from a prior
			// session (reconfig); (2) KioskMode — CREATE the custom desktop but leave it INACTIVE (it becomes the client's
			// startup desktop, not yet switched to); (3) ClientStart — SPAWN THE CLIENT EARLY so its cold start overlaps the
			// runtime lockdown ops below. The client comes up hidden on the still-inactive custom desktop, so early init is
			// inert-until-reveal and safe. The reveal (ActivateCustomDesktopOperation, far below) is gated on BOTH
			// LockdownApplied AND ClientReady, so the early client is NEVER shown before the lockdown has been applied.
			operations.Enqueue(new ClientTerminationOperation(dependencies, processFactory, proxyFactory, runtimeHost, THIRTY_SECONDS));
			var kioskModeOperation = new KioskModeOperation(dependencies, desktopFactory, desktopMonitor, explorerShell, processFactory);
			operations.Enqueue(kioskModeOperation);
			operations.Enqueue(new ClientStartOperation(dependencies, processFactory, runtimeHost, THIRTY_SECONDS));

			// Runtime lockdown operations — these run WHILE the client is cold-starting above.
			...
			// Lockdown is now applied: signal the reveal gate, then wait for the (early-spawned) client to be ready AND
			// establish the runtime<->client communication proxy before revealing.
			operations.Enqueue(new LockdownAppliedOperation(dependencies));
			operations.Enqueue(new ClientReadyOperation(dependencies, proxyFactory, runtimeHost, TWO_MINUTES));

			// #3/#4 reveal: switch to the kiosk desktop — fails closed unless BOTH LockdownApplied AND ClientReady are set,
			// so the desktop switch shows no black frame and never reveals a client before the lockdown is in place.
			operations.Enqueue(new ActivateCustomDesktopOperation(dependencies, kioskModeOperation));
