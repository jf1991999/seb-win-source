	...
	internal class ServiceOperation : SessionOperation
	{
		...
		private bool TryStartSession()
		{
			...
			if (communication.Success)
			{
				...
				else if (Context.Next.Settings.Service.Policy == ServicePolicy.Mandatory)
				{
					Logger.Error($"Failed to start new service session within {timeout_ms / 1000} seconds!");
				}
				else
				{
					// Defense-in-depth: the service acknowledged the start but never signalled session activation in
					// time (e.g. a service-side glitch). Since the service is not mandatory, degrade to a no-service
					// session rather than failing the whole launch and bricking the session. sessionId / serviceEventName
					// stay unset, so no StopSession is later attempted for a session that never actually started.
					Logger.Warn($"The service did not activate a session within {timeout_ms / 1000} seconds; continuing without a service session (service policy is not mandatory).");
					started = true;
				}
			}
			else
