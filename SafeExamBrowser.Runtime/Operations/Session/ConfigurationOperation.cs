...
namespace SafeExamBrowser.Runtime.Operations.Session
{
	internal class ConfigurationOperation : ConfigurationBaseOperation
	{
		...
		public override OperationResult Perform()
		{
			...
			Logger.Perf("Runtime: config loaded");
		...
		private bool TryInitializeSettingsUri(out Uri uri, out UriSource source)
		{
			...
			{
				var resource = commandLineArgs[1];

				// Blinkered: translate a 'blinkered://join?code=...&name=...' handoff into the https
				// class-join config URL and feed it through the normal config-loading path (the loader
				// treats it as a standard https resource - no duplication of the loader).
				if (BlinkeredUrl.TryRewriteJoin(resource, out var joinConfigUrl))
				{
					Logger.Info($"Translated 'blinkered://join' argument into configuration URL '{joinConfigUrl}'.");
					resource = joinConfigUrl;
				}

				isValidUri = Uri.TryCreate(resource, UriKind.Absolute, out uri);
