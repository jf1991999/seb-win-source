...
namespace SafeExamBrowser.Runtime.UnitTests.Operations.Session
{
	...
	public class ClientTerminationOperationTests
	{
		...
		public void MustDoNothingOnRepeatIfNoClientRunning()
		{
			...
			// Split teardown (ClientStartOperation.Revert, reused as ClientTerminationOperation.Repeat) defensively clears
			// the connection flag even when the client is already gone — assert that, and that nothing else was invoked.
			runtimeHost.VerifySet(h => h.AllowConnection = false, Times.Once);
