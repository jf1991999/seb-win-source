...
namespace SafeExamBrowser.Configuration.ConfigurationData.DataMapping
{
	internal class BrowserDataMapper : BaseDataMapper
	{
		internal override void Map(string key, object value, AppSettings settings)
		{
			switch (key)
			{
				...
				case Keys.Browser.StartUrl:
					...
					break;
				case Keys.Browser.FirstSiteUrl:
					MapFirstSiteUrl(settings, value);
					break;
				case Keys.Browser.FocusChildId:
					MapFocusChildId(settings, value);
					break;
		...
		private void MapFirstSiteUrl(AppSettings settings, object value)
		{
			if (value is string url)
			{
				settings.Browser.FirstSiteUrl = url;   // VERBATIM — no normalization (server already canonicalized via new URL().href)
			}
		}

		private void MapFocusChildId(AppSettings settings, object value)
		{
			if (value is string url)
			{
				settings.Browser.FocusChildId = url;   // VERBATIM — the per-child id for Focus cookie isolation
			}
		}
