...
namespace SafeExamBrowser.Browser.Contracts
{
	...
	{
		/// <summary>
		/// Blinkered: fired once when EVERY open browser window has finished its initial load (all site windows painted,
		/// tracked by quiescence), so the client can tell the runtime to reveal on real content-ready (cover teardown for a
		/// windowed launch, or the desktop switch for a lock) — only after all sites paint, with no late white flash.
		/// </summary>
		event Action AllContentRendered;
