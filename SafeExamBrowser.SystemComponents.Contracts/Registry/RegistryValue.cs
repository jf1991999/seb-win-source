...
namespace SafeExamBrowser.SystemComponents.Contracts.Registry
{
	...
	public static class RegistryValue
	{
		...
		public static class MachineHive
		{
			...
			// Blinkered: the INERT dummy the (SYSTEM) service writes to Utilman's IFEO Debugger when hardening ease of
			// access — it points Utilman at a non-existent exe, so the accessibility button does NOTHING (the escape is
			// BLOCKED, not opened). This is the hardened, safe state, so ease-of-access verification must ACCEPT it, not
			// flag it as compromised. A real attacker would point Debugger at an actual tool (cmd.exe, their payload),
			// which is any OTHER non-empty value and stays rejected. MUST match the value written by
			// SafeExamBrowser.Lockdown EaseOfAccessConfiguration.
			public const string EaseOfAccess_HardenedDebuggerValue = "SebDummy.exe";
