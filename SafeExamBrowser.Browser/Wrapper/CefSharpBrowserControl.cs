...
namespace SafeExamBrowser.Browser.Wrapper
{
	internal class CefSharpBrowserControl : ChromiumWebBrowser, ICefSharpControl
	{
		...
		public CefSharpBrowserControl(ILifeSpanHandler lifeSpanHandler, string url) : base(url)
		{
			...
			// Blinkered: receive messages the shared pages post via the window.webkit.messageHandlers
			// .blinkered shim (forwarded through CefSharp.PostMessage by Blinkered.js).
			JavascriptMessageReceived += BlinkeredBridge.OnJavascriptMessageReceived;
		}
