...
namespace SafeExamBrowser.Browser.Content
{
	internal class ContentLoader
	{
		...
		internal string LoadBlinkered()
		{
			if (blinkered == default)
			{
				var assembly = Assembly.GetAssembly(typeof(ContentLoader));
				var path = $"{typeof(ContentLoader).Namespace}.Blinkered.js";

				using (var stream = assembly.GetManifestResourceStream(path))
				using (var reader = new StreamReader(stream))
				{
					blinkered = reader.ReadToEnd();
				}
			}
