...
namespace SafeExamBrowser.Browser
{
	public class BrowserApplication : IBrowserApplication
	{
		...
		private readonly string lockdownGateEventName;
		...
		public event Action AllContentRendered;
		...
		public BrowserApplication(
			...
			string lockdownGateEventName = null)
		{
			...
			this.lockdownGateEventName = lockdownGateEventName;
		...
		public void Initialize()
		{
			...
			// Blinkered: let the bridge act on this application's live window collection.
			BlinkeredBridge.Logger = logger;
			BlinkeredBridge.Controller = new BlinkeredWindowController(() => Windows, logger) { FirstSiteUrl = context.Settings.FirstSiteUrl };

			// Blinkered launch reveal: give the bridge a callback so, once EVERY window has painted (all site windows'
			// LoadEnd, tracked by quiescence), it surfaces via this app's AllContentRendered event -> the client relays it
			// to the runtime, which reveals on real content-ready (cover teardown / desktop switch).
			BlinkeredBridge.OnAllContentRendered = () => AllContentRendered?.Invoke();
			...
			logger.Perf("Client: cef begin");

			// Blinkered Focus isolation is now purely via a per-child CEF cache dir (Cache-focus-<childId>, set as
			// CachePath in ConfigurationResponsibility). No pre-init cache wipe, no cookie snapshot/save — the OS keeps
			// each child's cookies + storage separate for free, which is what makes Focus launch reliably again.
			var success = Cef.Initialize(settings, true, default(IApp));

			logger.Perf("Client: cef done");
		...
		private void WaitForLockdownGate()
		{
			if (string.IsNullOrEmpty(lockdownGateEventName)) { return; }
			try
			{
				// TIMING hint only — the reveal is gated IN-PROCESS by the runtime, so on ANY failure/timeout we proceed.
				if (EventWaitHandle.TryOpenExisting(lockdownGateEventName, out var gate))
				{
					using (gate)
					{
						var signalled = gate.WaitOne(30000);
						logger.Info(signalled
							? "Lockdown gate signalled — proceeding with browser init."
							: "Lockdown gate wait timed out (30s) — proceeding with browser init anyway.");
					}
				}
				else
				{
					logger.Info("Lockdown gate not found — proceeding with browser init.");
				}
			}
			catch (System.Exception e)
			{
				logger.Warn($"Lockdown gate wait failed ({e.Message}) — proceeding with browser init.");
			}
		}
