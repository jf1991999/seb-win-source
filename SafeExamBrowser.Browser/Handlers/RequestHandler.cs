...
namespace SafeExamBrowser.Browser.Handlers
{
	internal class RequestHandler : CefSharp.Handler.RequestHandler
	{
		...
		protected override bool OnBeforeBrowse(IWebBrowser webBrowser, IBrowser browser, IFrame frame, IRequest request, bool userGesture, bool isRedirect)
		{
			...
			// Blinkered: a kid's page hands off into a class session by navigating to
			// blinkered://join?code=...&name=... (same-frame). Translate it to the https class-join config
			// URL and route it through SEB's EXISTING in-session reconfiguration path (download ->
			// reconfiguration-allowed policy -> session restart), exactly like navigating to a .seb config
			// URL. Caught before the request filter, since the custom scheme would otherwise be blocked.
			if (BlinkeredUrl.TryRewriteJoin(request.Url, out var joinConfigUrl))
			{
				logger.Info("Detected 'blinkered://join' navigation; routing into the in-session reconfiguration flow.");
				browser.GetHost().StartDownload(joinConfigUrl);
