...
namespace SafeExamBrowser.Browser.Handlers
{
	internal class KeyboardHandler : IKeyboardHandler
	{
		...
		private readonly AccentInput accentInput;

		internal KeyboardHandler(ILogger logger, IAccentPopup accentPopup)
		{
			accentInput = new AccentInput(logger, accentPopup);
		}
		...
		{
			// Press-and-hold accent input (Phase 0 spike): swallows accent-capable letters while held and
			// re-injects on a quick tap. Returns true only for the keys it consumes; everything else falls through.
			if (accentInput.OnPreKeyEvent(browser, type, keyCode, nativeKeyCode, modifiers))
			{
				return true;
			}
