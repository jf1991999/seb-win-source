...
namespace SafeExamBrowser.Browser
{
	internal class BrowserWindow : Contracts.IBrowserWindow
	{
		...
		/// <summary>The stable tab identity: the URL this window was OPENED for (never updated on navigation/redirect).</summary>
		internal string OriginatingUrl => context.OriginatingUrl;

		internal void SetOriginatingUrl(string url) => context.OriginatingUrl = url;
		...
		internal void Initialize()
		{
			...
			// Only spawn the real (WPF) accent popup when the feature is enabled; otherwise a harmless no-op, so the
			// default (gated-off) path creates no extra windows and carries zero new behaviour.
			var accentPopup = Environment.GetEnvironmentVariable("BLINKERED_ACCENT") == "1"
				? context.UserInterfaceFactory.CreateAccentPopup()
				: new NullAccentPopup();
			var keyboardHandler = new KeyboardHandler(logger.CloneFor($"{nameof(KeyboardHandler)} #{Id}"), accentPopup);
			...
			if (IsMainWindow)
			{
				...
				// Blinkered: construct on about:blank and defer the real start URL until the document-start bridge
				// shim is registered (in BrowserControl, once the browser is initialized) - so the first/home
				// navigation, which on a re-lock is a warm-cache instant load, can't race past the shim.
				cefSharpControl = new CefSharpBrowserControl(context.LifeSpanHandler, "about:blank");
			}
			else
			{
			...
			// Blinkered: the bridge shim is registered as a document-start script in BrowserControl. The main
			// window also carries the deferred start URL (loaded after the shim is registered); popups have none
			// (CEF drives their navigation).
			var blinkeredShim = new ContentLoader(context.Text).LoadBlinkered();
			var deferredStartUrl = IsMainWindow ? context.StartUrl : null;
			// Blinkered launch speedup: only the main window carries the first-site URL; popups get null (they never fire the early open).
			var firstSiteUrl = IsMainWindow ? context.Settings.FirstSiteUrl : null;
		...
		internal void Show()
		{
			...
			if (!IsMainWindow)
			{
				// Home mode: drop site windows below the home tab bar and make them chromeless. A 0 inset (e.g.
				// exam-mode popups, where the bridge is never driven) is a no-op. Done before Show() so the
				// chrome change happens before the window is displayed.
				var inset = BlinkeredBridge.Controller?.TopInset ?? 0;
				logger.Info($"[Blinkered] site window opening; applying top inset = {inset} (chromeless + offset when > 0).");
				Window.SetTopInset(inset);
			}
		...
		internal void SetTopInset(int height)
		{
			context.Window?.SetTopInset(height);
		}

		internal void SetAlwaysOnTop(bool alwaysOnTop)
		{
			context.Window?.SetAlwaysOnTop(alwaysOnTop);
		}
