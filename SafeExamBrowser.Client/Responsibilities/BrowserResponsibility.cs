	...
	internal class BrowserResponsibility : ClientResponsibility
	{
		...
		private void DeregisterEvents()
		{
			if (Browser != default)
			{
				...
				Browser.AllContentRendered -= Browser_AllContentRendered;
			}
		}
		...
		private void Browser_AllContentRendered()
		{
			// Blinkered launch reveal: every window has painted — tell the runtime so it reveals on real content-ready
			// (cover teardown for a windowed launch, or the desktop switch for a lock). Fire-and-forget OFF the browser
			// thread (this fires from CEF's LoadEnd) so a blocking IPC can't stall the browser; the fallback covers a miss.
			System.Threading.Tasks.Task.Run(() =>
			{
				try
				{
					runtime.InformContentReady();
				}
				catch (System.Exception e)
				{
					Logger.Warn($"Failed to inform the runtime that content is ready: {e.Message}");
				}
			});
		}
