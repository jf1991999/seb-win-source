...
namespace SafeExamBrowser.Client.Responsibilities
{
	internal class ShellResponsibility : ClientResponsibility
	{
		...
		{
			// #3: do NOT auto-promote the ActionCenter at launch. Promote() slid it in from the left (Left -Width -> 0),
			// held ~2s, then slid out — reading as a stray ⅓-screen panel animating in during launch. It stays fully
			// available via its edge activator; it just isn't shown unprompted while the session is coming up.
