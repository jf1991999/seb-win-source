...
namespace SafeExamBrowser.Client
{
	internal class ClientController
	{
		...
		internal bool TryStart()
		{
			...
			// Blinkered: the Runtime's launch cover (black + logo) is the sole launch visual — don't flash the client's
			// small splash card over it during startup (part of killing the "logo → black → logo" launch flicker). The
			// splash instance stays alive for error/dialog parenting; it is just never shown during the launch.
		...
		internal void Terminate()
		{
			...
			// Blinkered: do NOT flash the launch splash on teardown, for ANY mode. sp76 skipped it for windowed
			// (Focus — KioskMode.None); now extended to kiosk (Lock — CreateNewDesktop) too, because the horse-and-version
			// logo card added nothing but a flash on the way out. The Runtime already suppresses its own exit splash
			// (RuntimeController.Terminate "nothing on exit"), and a Lock's isolated-desktop teardown covers the screen via
			// its own desktop switch-back — so removing the card leaves only that (unavoidable) switch, not a bare desktop.
			// The splash instance stays alive as a parent for any shutdown-error message box and is closed below.
