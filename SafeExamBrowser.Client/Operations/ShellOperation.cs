...
namespace SafeExamBrowser.Client.Operations
{
	internal class ShellOperation : ClientOperation
	{
		...
		public override OperationResult Perform()
		{
			...
			logger.Perf("Client: shell begin");
		...
		{
			// #4 launch speedup: audio.Initialize() enumerates audio devices (NAudio) — ~1.15s on the launch critical
			// path. Only pay it when the volume control is actually surfaced (ShowAudio in the action center or taskbar).
			// If no volume control is shown (e.g. a kids' lock with no volume UI), skip it entirely — pure UI, no
			// security surface. (When shown, it's kept synchronous here so the control has the device at creation; if the
			// shown case turns out to matter for launch time, an async variant that fills the control via VolumeChanged
			// is the follow-up.)
			if (Context.Settings.UserInterface.ActionCenter.ShowAudio || Context.Settings.UserInterface.Taskbar.ShowAudio)
			{
				audio.Initialize();
			}
