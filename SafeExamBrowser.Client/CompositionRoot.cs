...
namespace SafeExamBrowser.Client
{
	internal class CompositionRoot
	{
		...
		private string lockdownGateEventName;
		...
		internal void BuildObjectGraph(Action shutdown)
		{
			...
			applicationMonitor = new ApplicationMonitor(TWO_SECONDS, ModuleLogger(nameof(ApplicationMonitor)), nativeMethods, processFactory, () => context.AppConfig?.CodeSignatureHash);
			networkAdapter = new NetworkAdapter(ModuleLogger(nameof(NetworkAdapter)));
			// Blinkered: create the client splash WITHOUT auto-showing it. The runtime's launch cover is the sole launch
			// surface; a client splash would activate on top of it (the client process starts after the cover, so its
			// topmost splash wins z-order). It is still created so an in-session reconfiguration can Show() it explicitly.
			splashScreen = uiFactory.CreateSplashScreen(show: false);
		...
		private void ValidateCommandLineArguments()
		{
			...
			if (hasFive)
			{
				...
				if (hasLogFilePath && hasLogLevel && hasHostUri && hasAuthenticationToken)
				{
					...
					// #A launch speedup — optional per-session nonce (args[6]) naming the runtime's browser-init gate. Timing
					// hint only; if absent the browser proceeds to init immediately.
					this.lockdownGateEventName = args.Length >= 7 ? $"Local\\Blinkered_LockdownGate_{args[6]}" : null;
