...
namespace SafeExamBrowser.UserInterface.Contracts.Browser
{
	...
	public interface IBrowserWindow : IWindow
	{
		...
		/// <summary>
		/// Reserves a strip of the given height (in device-independent pixels) at the top of the screen for the
		/// Blinkered home tab bar: an additional (non-main) window is made chromeless and repositioned to fill the
		/// area below the strip, so the tab bar in the main window stays visible. A height of zero (or the main
		/// window) is a no-op, leaving the window's default chrome and geometry unchanged.
		/// </summary>
		void SetTopInset(int height);

		/// <summary>
		/// Sets (or clears) the window's always-on-top state. In Blinkered home mode the site (additional) windows are
		/// kept always-on-top so the main window's brief activation on a tab-bar click can't surface its empty body
		/// above them (the black flash); the main window is raised into that band only while a home-page modal is
		/// shown (focusMainContent) and dropped back below the sites when a site is focused (focusWindow).
		/// </summary>
