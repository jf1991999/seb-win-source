...
namespace SafeExamBrowser.UserInterface.Contracts
{
	...
	public interface IUserInterfaceFactory
	{
		...
		/// <summary>
		/// Creates the press-and-hold accent popup (or a no-op for user interfaces that do not provide it).
		/// </summary>
		IAccentPopup CreateAccentPopup();
		...
		/// <summary>
		/// Creates a launch-only full-screen black cover (shown behind the splash at startup, torn down at
		/// content-ready) so a windowed Focus launch goes straight to black instead of flashing the home desktop.
		/// </summary>
		IWindow CreateLaunchCover(AppConfig appConfig = null);

		/// <summary>
		/// Creates a new splash screen which runs on its own thread.
		/// </summary>
		ISplashScreen CreateSplashScreen(AppConfig appConfig = null, bool show = true);
