...
namespace SafeExamBrowser.UserInterface.Desktop.Windows
{
	internal partial class RuntimeWindow : Window, IRuntimeWindow
	{
		...
		private void InitializeRuntimeWindow()
		{
			...
			// Version + build only — NO ETH/SEB copyright on the launch window (kept in AssemblyCopyright for MPL).
			InfoTextBlock.Inlines.Add(new Run($"Version {appConfig.ProgramInformationalVersion}"));
			InfoTextBlock.Inlines.Add(new LineBreak());
			InfoTextBlock.Inlines.Add(new Run($"Build {appConfig.ProgramBuildVersion}") { FontSize = 9, Foreground = Brushes.Gray });
