...
namespace SafeExamBrowser.UserInterface.Desktop.Windows
{
	internal partial class Taskbar : Window, ITaskbar
	{
		...
		// #3: launch suppression — the taskbar sits off-screen until the main browser content is revealed.
		private bool pendingTaskbarReveal;
		private double taskbarTop;
		private System.Windows.Threading.DispatcherTimer taskbarRevealSafetyNet;
		...
		public void InitializeBounds()
		{
			Dispatcher.Invoke(() =>
			{
				...
				Left = 0;
				taskbarTop = SystemParameters.PrimaryScreenHeight - Height;
				Top = taskbarTop;

				// #3: don't let the taskbar chrome flash on the bare desktop before the lock page is up. Sit off-screen
				// (just below the screen) until the main browser content is revealed, then slide up with it. If that
				// reveal already happened (or never comes), the safety net brings the taskbar in regardless.
				if (!BrowserWindow.HasMainContentRevealed)
				{
					pendingTaskbarReveal = true;
					Top = SystemParameters.PrimaryScreenHeight + Height;
					BrowserWindow.MainContentRevealed += OnMainContentRevealed;

					taskbarRevealSafetyNet = new System.Windows.Threading.DispatcherTimer { Interval = TimeSpan.FromSeconds(10) };
					taskbarRevealSafetyNet.Tick += (o, e) => RevealTaskbar();
					taskbarRevealSafetyNet.Start();
				}
		...
		private void OnMainContentRevealed()
		{
			Dispatcher.Invoke(RevealTaskbar);
		}

		private void RevealTaskbar()
		{
			if (!pendingTaskbarReveal)
			{
				return;
			}

			pendingTaskbarReveal = false;
			taskbarRevealSafetyNet?.Stop();
			BrowserWindow.MainContentRevealed -= OnMainContentRevealed;
			Top = taskbarTop;   // slide up into the bottom of the screen, together with the content
			logger.Debug("Taskbar revealed alongside the main browser content.");
		}
