...
namespace SafeExamBrowser.Service.UnitTests
{
	...
	public class ServiceControllerTests
	{
		...
		[TestMethod]
		public void Communication_MustSupersedeStaleSessionOnStart()
		{
			// A session is still marked running because the previous Runtime was force-killed without a clean stop.
			// A new SessionStart must supersede it: revert the stale session (restoring lockdown / ease-of-access),
			// THEN perform the new one - not be ignored (which left the ServiceEvent unset and timed out the Runtime).
			var args = new SessionStartEventArgs { Configuration = new ServiceConfiguration { SessionId = Guid.NewGuid() } };
			var order = 0;
			var revert = 0;
			var perform = 0;
			...
			bootstrapSequence.Setup(b => b.TryPerform()).Returns(OperationResult.Success);
			sessionSequence.Setup(s => s.TryRevert()).Returns(OperationResult.Success).Callback(() => revert = ++order);
			sessionSequence.Setup(s => s.TryPerform()).Returns(OperationResult.Success).Callback(() => perform = ++order);
			...
			sessionSequence.Verify(s => s.TryRevert(), Times.Once);    // stop the stale session
			sessionSequence.Verify(s => s.TryPerform(), Times.Once);   // then start the new one

			Assert.AreEqual(1, revert);                                // stop BEFORE start
			Assert.AreEqual(2, perform);
			Assert.AreEqual(args.Configuration.SessionId, sessionContext.Configuration.SessionId);   // new config applied
		}
