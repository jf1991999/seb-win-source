...
namespace SafeExamBrowser.Communication.Contracts.Data
{
	...
	public enum SimpleMessagePurport
	{
		...
		/// <summary>
		/// Blinkered: sent from the client to the runtime when the first-site browser window has finished rendering
		/// (CEF LoadEnd), so the runtime can tear down the launch cover on real content-ready rather than at session
		/// start (avoids revealing the white/staging assembly).
		/// </summary>
		ContentIsReady,
